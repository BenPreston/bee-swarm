  ...
  if (this.img) {
    ...
    for (var y = 0; y < height; y++) {
      for (var x = 0; x < width; x++) {
        ...
        if (alpha > 0) {
          self.visiblePixels.push(
            new Point(
              x * self.opts.spread + self.opts.offset.x,
              y * self.opts.spread + self.opts.offset.y
            )
          );
        }
      }
    ...
    for (var x = 0; x < width; x++) {
      for (var y = 0; y < height; y++) {
        self.visiblePixels.push(
          new Point(
            x * self.opts.spread + this.opts.offset.x,
            y * self.opts.spread + this.opts.offset.y
          )
        );
      }
    }
