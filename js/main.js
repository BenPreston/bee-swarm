let canvas, canvas_dom;
let shapes, piece_collection;
let renderer;
let current_shape = "blankcanvas";

$(function () {
  function onShapeChange(shapeName) {
    var shape = shapes.get(shapeName);
    if (shape) {
  ...
  $("button").click(function () {
    const shape_name = $(this).data("shape");
    onShapeChange(shape_name);
  });
...
function toShape(shape, opts) {
  ...
  for (var i = 0; i < piece_collection.pieces.length; i++) {
    ...
    if (_opts.random && _opts.range > 0) {
      speed =
        _opts.speed -
        Math.round(_opts.range / 2) +
        Helpers.Random.getRandomIntInclusive(0, _opts.range);
    }
